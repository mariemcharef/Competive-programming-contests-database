...
* Competitions, problems for ech competition, including basic identifying information
* Problems details, which includes basic information about problems(topics, test_cases..)
* Participants submissions, including the time at which the submission was made, the judgement, rank and score it received, and the problem to which the submission is related
* Clarifications from participantss, including the content of the clarification .
* Announcements made by the competition creator to one competition.
...
Per the typical queries in `queries.sql`, it is common for users of the database to access all submissions submitted by any particular participant. For that reason, indexes are created on the `name` column to speed the identification of participants by those columns.

Similarly, it is also common practice for a user of the database to concerned with viewing all participants who make subbmissions to a particular problem. As such, an index is created on the `name` column in the `problems` table to speed the identification of problems by name.

And, it is common practice for a database user to view the members of one team(participant with more tha one user), a view is created to join ech paricipant(where users > 1) with users table.
